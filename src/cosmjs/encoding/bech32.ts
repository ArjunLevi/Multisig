...
export function toBech32(prefix: string, data: Uint8Array, limit?: number): string {
  const address = bech32.bech32.encode(prefix, bech32.bech32.toWords(data), limit);
  return address;
}
...
export function fromBech32(
  ...
  const decodedAddress = bech32.bech32.decode(address, limit);
  return {
    prefix: decodedAddress.prefix,
    data: new Uint8Array(bech32.bech32.fromWords(decodedAddress.words)),
  };
}
